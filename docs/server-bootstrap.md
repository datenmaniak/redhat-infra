# Server bootstrap for Red Hat infrastructure Laboratory
...
### 2. Copia/Pega el script en `setup-redhat-lab-fixed.sh`
...
# Verificar que la red default existe
# if ! virsh net-list --all | grep -q "default"; then
#   echo "ERROR: La red 'default' no existe. Créala primero:"
#   echo "  virsh net-define /tmp/default-net.xml"
...
cp -f alma9-base.qcow2 alma9-seed.qcow2
virt-sysprep -a alma9-seed.qcow2
virt-customize -a alma9-seed.qcow2 \
...
create_vm() {
  ...
  sudo qemu-img create -f qcow2 -b alma9-seed.qcow2 -F qcow2 "$VM_DIR/${name}.qcow2" "${disk}G"
...
EOF
  ...
  sudo genisoimage -output "$VM_DIR/${name}-cidata.iso" -volid cidata -joliet -rock \
...
create_vm "alma-rhcsa" 3072 2 40 "" ""
...
create_vm "alma-security" 3096 2 50 "" ""
...
### 3.  Iniciar bootstraping de las VMS
